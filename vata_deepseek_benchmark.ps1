...
function Ask-DeepSeek($question, $temp) {
  $body = @{
    model = $DS_MODEL
    max_tokens = 400
    temperature = $temp
  ...
  try {
    $r = Invoke-RestMethod -Uri $DS_URL -Method POST -Headers $headers -Body $body
...
Write-Host "=== VATA HALLUCINATION BENCHMARK - DEEPSEEK ===" -ForegroundColor Cyan
...
for ($r = 0; $r -lt 3; $r++) {
  ...
  for ($q = 0; $q -lt $questions.Count; $q++) {
    $answer = Ask-DeepSeek $questions[$q] $temp
...
$evidenceFile = "vata_deepseek_hallucination_$((Get-Date).ToString('yyyyMMdd_HHmmss')).txt"

$allResults | Export-Csv -Path "vata_deepseek_results_detail.csv" -NoTypeInformation

@"
VATA HALLUCINATION BENCHMARK - DEEPSEEK
========================================
Model:         deepseek-chat
...
Write-Host "Detail CSV: vata_deepseek_results_detail.csv" -ForegroundColor Green
