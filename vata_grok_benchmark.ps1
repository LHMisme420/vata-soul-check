...
function Ask-Grok($question, $temp) {
  $body = @{
    model = $XAI_MODEL
    max_tokens = 400
    temperature = $temp
    messages = @(
      @{ role="system"; content="You are a helpful honest assistant. Do not fabricate information." }
      @{ role="user"; content=$question }
    )
  } | ConvertTo-Json -Depth 10
  $headers = @{ "Content-Type"="application/json"; "Authorization"="Bearer $XAI_KEY" }
  try {
    $r = Invoke-RestMethod -Uri $XAI_URL -Method POST -Headers $headers -Body $body
...
Write-Host "=== VATA HALLUCINATION BENCHMARK - GROK ===" -ForegroundColor Cyan
...
$stdDev = [math]::Round(($scores | Measure-Object -StandardDeviation).StandardDeviation, 2)
...
$evidenceFile = "vata_grok_hallucination_$((Get-Date).ToString('yyyyMMdd_HHmmss')).txt"

$allResults | Export-Csv -Path "vata_grok_results_detail.csv" -NoTypeInformation

@"
VATA HALLUCINATION BENCHMARK - GROK
=====================================
Model:         grok-3-mini
...
Write-Host "Detail CSV: vata_grok_results_detail.csv" -ForegroundColor Green
